...
- `game.rs` — `GameState`, `Phase` enum, `Wind` (zero-crossing swing: 12·sin(0.03t+slow_phase) + 2·sin(0.07t+fast_phase), envelope ±14 m/s; the old frozen per-round base left most rounds one-signed forever), `PlayerCannon`, `DefenderCannon`; damage/reload tuning (`AOE_R`, `SEG_DMG`, `CANNON_DMG`, `RELOAD`, `CHARGE_STEP`), `END_SLOWMO`/`END_HOLD`, `contact()`, `fresh_seed()`.
