...
- `lib.rs` — `run()` game loop: dt capped 0.05, `frame_input()` (only macroquad input poll; maps cursor via `render::screen_to_world`, draws `fresh_seed()` from the platform clock) → `GameState::update(dt, &FrameInput, &mut Audio)` → `render::draw`, MedievalSharp font via `include_bytes!` (silent fallback to default font on load failure).
...
- `game.rs` — `GameState`, `Phase` enum, `Wind` (zero-crossing swing: 12·sin(0.03t+slow_phase) + 2·sin(0.07t+fast_phase), envelope ±14 m/s; the old frozen per-round base left most rounds one-signed forever), `PlayerCannon`, `DefenderCannon`; damage/reload tuning (`AOE_R`, `SEG_DMG`, `CANNON_DMG`, `RELOAD`, `CHARGE_STEP`), `END_SLOWMO`/`END_HOLD`, `contact()`. Macroquad-free since the FrameInput refactor: input arrives as `game::FrameInput` (world-space `aim`, `wheel`, `click`/`fire`/`pause`/`restart` intents, `restart_seed`), restart takes a seed parameter — so the sim runs headless for tests/AI training; `fresh_seed()` lives in `lib.rs`.
